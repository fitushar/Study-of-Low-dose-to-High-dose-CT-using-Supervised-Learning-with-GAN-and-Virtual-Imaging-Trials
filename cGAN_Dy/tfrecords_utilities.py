...
@tf.function
def decode_dual_ct(Serialized_example):
    features = {
                'covid_lbl': tf.io.FixedLenFeature([],tf.int64),
                'lower_img': tf.io.FixedLenFeature([],tf.string),
                'medium_img':tf.io.FixedLenFeature([],tf.string),
    ...
    image_1     = tf.io.decode_raw(examples['lower_img'], float) ##Decode_image_float
    input_image1 = tf.reshape(image_1,img_shape)        #Resgapping_the_data
    input_image1 = tf.expand_dims(input_image1, axis=-1) #Because CNN expect(batch,H,W,D,CHANNEL)
    input_image1 = tf.cast(input_image1, tf.float32)     #Because CNN expect(batch,H,W,D,CHANNEL)



    image_2     = tf.io.decode_raw(examples['medium_img'], float) ##Decode_image_float
    input_image2 = tf.reshape(image_2,img_shape)        #Resgapping_the_data
    input_image2 = tf.expand_dims(input_image2, axis=-1) #Because CNN expect(batch,H,W,D,CHANNEL)
    input_image2 = tf.cast(input_image2, tf.float32)     #Because CNN expect(batch,H,W,D,CHANNEL)

    input_image = tf.concat([input_image1,input_image2],axis=-1)


    image_3    = tf.io.decode_raw(examples['high_img'], float) ##Decode_image_float
    real_image = tf.reshape(image_3,img_shape)        #Resgapping_the_data
    real_image = tf.expand_dims(real_image, axis=-1) #Because CNN expect(batch,H,W,D,CHANNEL)
    real_image = tf.cast(real_image, tf.float32)     #Because CNN expect(batch,H,W,D,CHANNEL)
