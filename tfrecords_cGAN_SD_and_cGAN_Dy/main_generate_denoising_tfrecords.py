...
def generate_denoising_tfrecords():
    ...
    medium_dose_ct_name_list = read_csv[IMAGE_PATH_INDEX_NAME_medium].tolist()
    ...
    for ct_data_i in range(df_start,df_end):
        ...
        img_path_medium    = RAW_DATA_DIRECTORY_medium + medium_dose_ct_name_list[ct_data_i]
        ...
        #--|CT-medium
        img_sitk_medium = sitk.ReadImage(img_path_medium, sitk.sitkFloat32)
        img_sitk_medium = resample_img2mm(img_sitk_medium, out_spacing=[2.0, 2.0, 1.0], is_label=False)
        image_medium    = sitk.GetArrayFromImage(img_sitk_medium)
        if FLIP_CT=='True':
           image_medium= image_medium[:, ::-1, :]
        image_medium    = np.clip(image_medium, -1000., 500.).astype(np.float32)
        image_medium    = normalise_one_one(image_medium)
        print('CT-medium Preprocessing done !')
        ...
        print('CT-medium-Shape---{}'.format(image_medium.shape))
        ...
        for ct_slice_j in range(30,345):
            ...
            slice_medium =  image_medium[ct_slice_j,:,:]
            ...
            slice_medium = resize_image_with_crop_or_pad(slice_medium, patch_params['example_size'], mode='constant',constant_values=patch_params['padding_value'])
            ...
            with tf.io.TFRecordWriter(record_mask_file) as writer:
                feature = {
                               'covid_lbl': _int64_feature(diseased_lbl),
                                ...
                                'medium_img':_bytes_feature(slice_medium.tostring()),
